...
class ModelEvaluator:
    ...
    def get_not_interacted_items_sample(self, person_id, sample_size, seed=42):
        interacted_items = self.get_items_interacted(
            person_id, self.dataset.interactions_full_indexed_df
        )
        all_items = set(self.dataset.articles_df["contentId"])
        non_interacted_items = all_items - interacted_items

        random.seed(seed)
        non_interacted_items_sample = random.sample(
            sorted(non_interacted_items), sample_size
        )
    ...
    def evaluate_model_for_user(self, model, person_id):
        ...
        interacted_values_testset = self.dataset.interactions_test_indexed_df.loc[
            person_id
        ]
        ...
        person_recs_df = model.recommend_items(
            ...
            items_to_ignore=self.get_items_interacted(
                person_id, self.dataset.interactions_train_indexed_df
            ),
            topn=10000000000,
        )
    ...
    def evaluate_model(self, model):
        ...
        for idx, person_id in enumerate(
            list(self.dataset.interactions_test_indexed_df.index.unique().values)
        ):
    ...
    def evaluate_models(self, models):
        # calculate the global metrics for each model and combine the detailed results as dataframe
        global_metrics = []
        detailed_results = []
        for model in models:
            global_metric, detailed_result = self.evaluate_model(model)
            global_metrics.append(global_metric)
            detailed_results.append(detailed_result)
            print(model.get_model_name(), global_metric)
        df = pd.DataFrame(global_metrics)
        df.set_index("modelName", inplace=True)
        return df

    def evaluate_models_report(self, models):
        df = self.evaluate_models(models)
