...
class Dataset(object):
    ...
    def read_dataset(self):
        ...
        self.interactions_df = pd.read_csv(self.data_directory+'users_interactions.csv')
    ...
    def train_test_split(self, test_ratio: float=0.20, random_state=42)->None:
        interactions_train_df, interactions_test_df = train_test_split(self.interactions_full_df,
    ...
    def build_index(self)-> None:
        ...
        self.interactions_full_indexed_df = self.interactions_full_df.set_index('personId')
        self.interactions_train_indexed_df = self.interactions_train_df.set_index('personId')
        self.interactions_test_indexed_df = self.interactions_test_df.set_index('personId')
        
    def __repr__(self)->str:
        def df_info(df: pd.DataFrame, df_name: str) -> str:
            return f"{df_name}: \nshape: {df.shape} \n{df.dtypes}\n"
        return '\n'.join([df_info(df, df_name) for df, df_name in [
