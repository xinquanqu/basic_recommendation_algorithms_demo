...
class ContentBasedRecommender:
    ...
    def build_token_embeddings(self):
        ...
        self.tfidf_matrix = vectorizer.fit_transform(
            self.dataset.articles_df["title"] + "" + self.dataset.articles_df["text"]
        )
    ...
    def build_user_profiles(self):
        ...
        def build_users_profile(person_id, interactions_indexed_df):
            ...
            user_item_strengths_weighted_avg = np.sum(
                user_item_profiles.multiply(user_item_strengths), axis=0
            ) / np.sum(user_item_strengths)
            user_item_strengths_weighted_avg = np.asarray(
        ...
        interactions_indexed_df = interactions_train_df[
            interactions_train_df["contentId"].isin(
                self.dataset.articles_df["contentId"]
            )
        ].set_index("personId")
        user_profiles = {}
