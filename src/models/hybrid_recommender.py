class HybridRecommender:
    ...
    def __init__(
        self, cb_rec_model, cf_rec_model, cb_ensemble_weight=1.0, cf_ensemble_weight=1.0
    ):
    ...
    def recommend_items(self, user_id, items_to_ignore=[], topn=10, verbose=False):
        ...
        recs_df = cb_recs_df.merge(
            cf_recs_df, how="outer", left_on="contentId", right_on="contentId"
        ).fillna(0.0)
        ...
        recommendations_df = recs_df.sort_values(
            "recStrengthHybrid", ascending=False
        ).head(topn)
