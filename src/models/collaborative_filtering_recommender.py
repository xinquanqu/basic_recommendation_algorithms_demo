...
class CFRecommender:
    ...
    def build_user_item_embeddings_with_svd(self):
        ...
        users_items_pivot_matrix_df = self.dataset.interactions_train_df.pivot(
            index="personId", columns="contentId", values="eventStrength"
        ).fillna(0)
        ...
        U, sigma, Vt = svds(
            users_items_pivot_sparse_matrix, k=self.NUMBER_OF_FACTORS_MF
        )
    ...
    def user_predicted_ratings(self):
        ...
        self.cf_predictions_df = pd.DataFrame(
            all_user_predicted_ratings_norm,
            columns=self.users_items_pivot_matrix_df.columns,
